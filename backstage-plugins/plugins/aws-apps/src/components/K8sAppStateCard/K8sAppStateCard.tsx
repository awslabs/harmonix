...
const OpaAppStateOverview = ({
  ...
  const getDeploymentEnvVars = (deploymentName: string): KeyValue[] => {
    ...
    const configMapName = appStateData.filter(appState => appState.appID === deploymentName)[0].stateObject.spec
      .template.spec.containers[0]?.envFrom?.[0]?.configMapRef?.name;
        ...
        <>
          {envVarArr.map(envVar => (
            <TableRow key={envVar.id}>
              <TableCell>
                <Typography sx={{ fontWeight: 'bold' }}>{envVar.key}</Typography>
              </TableCell>
    ...
    return (
      <>
        <Grid container sx={{ marginBottom: 2 }}>
          <Grid item xs={6}>
            <Typography sx={{ textTransform: 'uppercase', fontWeight: 'bold', paddingBottom: '10px' }}>
                      ...
                      <TableBody>
                        ...
                        <TableRow key="status">
                          ...
                          <TableCell id="appStatus">
                            {deploymentState?.appState ? deploymentState?.appState : 'Not Running'}
                          </TableCell>
                        </TableRow>
                        <TableRow key="pods">
                          <TableCell id="id" width="30%">
                            <Typography sx={{ fontWeight: 'bold' }}>Pods</Typography>
                          </TableCell>
                          <TableCell id="providerName">
                            {deploymentState?.runningCount + '/' + deploymentState?.desiredCount}
                            {deploymentState?.pendingCount ? ` (${deploymentState?.pendingCount} Pending)` : ''}
                          </TableCell>
                        </TableRow>
                        <TableRow key="lastUpdated">
                          ...
                          <TableCell id="providerName">
                            {deploymentState?.lastStateTimestamp ? deploymentState?.lastStateTimestamp.toString() : ''}
                          </TableCell>
                        </TableRow>
                      </TableBody>
                    </Table>
            ...
            {index === total ? (
              <>
                ...
                <Typography fontStyle={'italic'} fontSize={'12px'} sx={{ mt: 1 }}>
                  {' '}
                  **Changes to your application state will be applied directly to the cluster and not to the source code
                  repository
                </Typography>
              </>
            ) : (
              ...
              <Grid item xs={11}>
                <Table size="small" padding="none">
                  <TableBody>
                    <TableRow key="clusterName">
                      <TableCell id="id" width="30%">
                        <Typography sx={{ fontWeight: 'bold' }}>Cluster Name</Typography>
                      </TableCell>
                      <TableCell id="providerName">{clusterNameState}</TableCell>
                    </TableRow>
                    <TableRow key="namespace">
                      <TableCell id="id" width="30%">
                        <Typography sx={{ fontWeight: 'bold' }}>Namespace</Typography>
                      </TableCell>
