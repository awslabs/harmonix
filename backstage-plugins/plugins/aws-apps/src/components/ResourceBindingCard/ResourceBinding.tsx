...
const ResourceBindingCard = ({
  ...
  async function getBindingDetails() {
    ...
    const resourcesEntities = await Promise.all(
      resourceRefs.map(async (entityRef: { targetRef: string | CompoundEntityRef }) => {
      ...
      <CardContent>
              ...
              return (
                <TableRow key={record.id}>
                  ...
                  <TableCell id="resourceName">
                    <EntityRefLink entityRef={record.entityRef!} title={record.resourceName} />{' '}
                  </TableCell>
        ...
        <Typography margin={'10px'}>
          ...
          <ResourceSelectorDialog
            ...
            currentEnvironment={awsComponent.currentEnvironment.environment.name}
            catalog={catalog}
            isOpen={openDialog}
