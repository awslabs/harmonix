...
export const useAwsComponentFromContext = (): AwsComponentHookLoadingStatus => {
  ...
  async function getCatalogData(): Promise<EnvEntityMap> {
    ...
    const envProviderRefs = envEntities
      ...
      .map(envLookupResponse => {
        const envProviderRefs: EntityRelation[] | undefined = envLookupResponse.entity?.relations?.filter(
      ...
      if (defaultDeployStatus === ExtraStackDeployStatus.STAGED) {
        ...
        if (stackDetails.Outputs) {
          const logGroupsArrayOutput =
            stackDetails.Outputs.filter(output => output.OutputKey === 'LogGroupsArray')[0] ?? null;
          if (logGroupsArrayOutput) {
            svAppDeployEnv.app.logGroupNames.push(...JSON.parse(logGroupsArrayOutput.OutputValue as string));
          }

          const apiUrlOutput =
            stackDetails.Outputs.filter(output => output.OutputKey === 'ApiGatewayEndpointWithPath')[0] ?? null;
          if (apiUrlOutput) {
  ...
  async function getData(): Promise<AWSComponent> {
    ...
    const deployEnvs: AwsDeploymentEnvironments = envEntities.reduce((acc, envEntity) => {
      ...
      if (envEntity.envName === _change_to_env_name) {
        envProvider = envProviders.filter(
          (providerEntity: Entity) => providerEntity.metadata.name === _change_to_env_provider_name,
        )[0];
    ...
    function getLowerEnvironment(envs: AwsDeploymentEnvironments): GenericAWSEnvironment {
      ...
      Object.values(envs).forEach(env => {
        if (lowest && lowest.environment.level > env.environment.level) lowest = env;
      });
      return lowest;
    }
    ...
    if (
      awsComponent.currentEnvironment.providerData.providerType === ProviderType.SERVERLESS &&
    ...
    if (
      awsComponent.currentEnvironment.providerData.providerType === ProviderType.GENAI_SERVERLESS &&
      componentType === 'aws-app'
    ) {
  ...
  const { value: component, error, loading, retry: refresh } = useAsyncRetry(() => getData(), [catalogApi, entity]);
...
export const AsyncAwsAppProvider = (props: AsyncAwsAppProviderProps) => {
  ...
  return <AwsAppContext.Provider value={value}>{children}</AwsAppContext.Provider>;
};
