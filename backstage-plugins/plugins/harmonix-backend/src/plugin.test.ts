...
import { harmonixPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [harmonixPlugin],
    });

    await request(server).get('/api/harmonix/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/harmonix/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/harmonix/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/harmonix/todos/${createdTodoItem.id}`)
