...
# that is installed. 
# 0.6.0 will install Backstage v1.37.0
...
if [[ "$installMode" == "from-source" ]]; then
    ...
    AWS_APPS_VERSION=^$(cat $opaHomeDir/backstage-plugins/plugins/harmonix-frontend/package.json | jq -r '.version')
    AWS_APPS_BACKEND_VERSION=^$(cat $opaHomeDir/backstage-plugins/plugins/harmonix-backend/package.json | jq -r '.version')

    AWS_APPS_SCAFFOLDER_VERSION=^$(cat $opaHomeDir/backstage-plugins/plugins/scaffolder-backend-module-harmonix/package.json | jq -r '.version')
    AWS_APPS_CATALOG_PROCESS_VERSION=$(cat $opaHomeDir/backstage-plugins/plugins/catalog-backend-module-harmonix/package.json | jq -r '.version')
fi
