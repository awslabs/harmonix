...
if [[ "$installMode" == "from-source" ]]; then
    cp -R $harmonixHomeDir/backstage-plugins/ $backstageDir
    rm -f $backstageDir/plugins/yarn.lock
    ...
    AWS_APPS_VERSION=^$(cat $harmonixHomeDir/backstage-plugins/plugins/aws-apps/package.json | jq -r '.version')
    AWS_APPS_BACKEND_VERSION=^$(cat $harmonixHomeDir/backstage-plugins/plugins/aws-apps-backend/package.json | jq -r '.version')
    AWS_APPS_DEMO_VERSION=^$(cat $harmonixHomeDir/backstage-plugins/plugins/aws-apps-demo/package.json | jq -r '.version')
    AWS_APPS_SCAFFOLDER_VERSION=^$(cat $harmonixHomeDir/backstage-plugins/plugins/scaffolder-backend-module-aws-apps/package.json | jq -r '.version')
    AWS_APPS_CATALOG_PROCESS_VERSION=$(cat $harmonixHomeDir/backstage-plugins/plugins/catalog-backend-module-aws-apps-entities-processor/package.json | jq -r '.version')
fi
...
# Note that these modifications were based on modifying Backstage 1.37.1 files.  
