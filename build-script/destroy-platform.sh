...
if [[ -z $AWS_DEFAULT_REGION ]]; then
  ...
  aws_region AWS_REGION "Enter the AWS region where the OPA platform was deployed:" "$defaultRegion"
fi

cd $opaPlatformDir
confirm_aws_account
cdk destroy --account $AWS_ACCOUNT_ID --region $AWS_REGION --progress bar
cd -
