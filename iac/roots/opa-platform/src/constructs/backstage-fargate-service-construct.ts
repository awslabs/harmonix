...
export class BackstageFargateServiceConstruct extends Construct {
  ...
  constructor(
    scope: Construct,
    id: string,
    props: BackstageFargateServiceConstructProps
  ) {
    super(scope, id);
    ...
    const backstageSecret = new Secret(
      this,
      `${props.opaEnv.prefix}-backstage-appsecret`,
      {
      ...
      [
        {
          ...
          reason:
            "Application secret for service-to-service auth and cannot be automatically rotated",
        },
      ]
    );
    ...
    lbDNSParam = new ssm.StringParameter(
      this,
      `${props.opaEnv.prefix}-backstage-hostname`,
      {
        allowedPattern: ".*",
      ...
      `${props.opaEnv.prefix}-backstage`,
      {
        ...
        taskImageOptions: {
          ...
          secrets: {
            ...
            OKTA_ORG_URL: ecs.Secret.fromSecretsManager(
              props.oktaSecret,
              "audience"
            ),
            OKTA_CLIENT_ID: ecs.Secret.fromSecretsManager(
              props.oktaSecret,
              "clientId"
            ),
            OKTA_CLIENT_SECRET: ecs.Secret.fromSecretsManager(
              props.oktaSecret,
              "clientSecret"
            ),
            OKTA_API_TOKEN: ecs.Secret.fromSecretsManager(
              props.oktaSecret,
              "apiToken"
            ),
            BACKSTAGE_SECRET: ecs.Secret.fromSecretsManager(backstageSecret),
            GITLAB_ADMIN_TOKEN: ecs.Secret.fromSecretsManager(
              props.gitlabAdminSecret,
              "apiToken"
            ),
          },
          containerPort: 8080,
    ...
    const cfnEcsService = albFargateService.service.node
      .defaultChild as ecs.CfnService;
    cfnEcsService.desiredCount = 0;

    NagSuppressions.addResourceSuppressions(
      albFargateService.taskDefinition,
      [
        {
          id: "AwsSolutions-ECS2",
          reason:
            "Task environment variables are not sensitive.  When changed, a new task def will be required for a new container",
        },
        {
          id: "AwsSolutions-IAM5",
          reason:
            "Task def exec role restricts access to the repository via additional policies, but requires multiple resources access to GetAuthorizationToken.",
          appliesTo: ["Resource::*"],
        },
      ],
      true
    ...
    if (props.ecrKmsKey) {
      props.ecrKmsKey.grantDecrypt(
        albFargateService.taskDefinition.executionRole!
      );
    }
    ...
    props.dbCluster.connections.allowDefaultPortFrom(
      ...
      "from fargate service"
    );

    // allow traffic to the ALB from the restricted IP security group
    albFargateService.loadBalancer.connections.addSecurityGroup(
      props.network.allowedIpsSg
    );
