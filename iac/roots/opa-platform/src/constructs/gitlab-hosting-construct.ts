...
export class GitlabHostingConstruct extends Construct {
  ...
  constructor(
    parent: Construct,
    name: string,
    props: GitlabHostingConstructProps
  ) {
    super(parent, name);
    ...
    const albSecurityGroup = new ec2.SecurityGroup(this, "gitlab-alb-sg", {
      vpc: props.network.vpc,
    });

    new ssm.StringParameter(
      this,
      `${props.opaEnv.prefix}-gitlab-security-group`,
      {
        allowedPattern: ".*",
    ...
    const gitlabRunnerSecurityGroup = new ec2.SecurityGroup(
      this,
      glRunnerSgName,
      {
        vpc: props.network.vpc,
    ...
    NagSuppressions.addResourceSuppressions(gitlabHost, [
      {
        ...
        reason:
          "Gitlab is a 3rd party scm used by the prototype to simulate customer environment.  Customer environment will not use EC2 and, therefore, detailed monitoring and incurring extra cost is not required.",
      },
      {
        id: "AwsSolutions-EC29",
        reason:
          "Gitlab is a 3rd party scm used by the prototype to simulate customer environment.  Autoscaling is not required, nor appropriate for this scm implementation.",
      },
    ]);
    ...
    // create ALB
    const gitlabAlb = new elb.ApplicationLoadBalancer(
      this,
      `${props.opaEnv.prefix}-gitlab-alb`,
      {
        vpc: props.network.vpc,
        vpcSubnets: { subnetType: ec2.SubnetType.PUBLIC },
    ...
    const aRecord = new route53.ARecord(
      this,
      `${props.opaEnv.prefix}-r53-aliasrecord`,
      {
        zone: props.hostedZone?.hostedZone!,
        target: route53.RecordTarget.fromAlias(
          new route53Targets.LoadBalancerTarget(gitlabAlb)
        ),
    ...
    const httpsListener = gitlabAlb.addListener(
      `${props.opaEnv.prefix}-alb-httpsListener`,
      {
    ...
    lbDNSParam = new ssm.StringParameter(
      this,
      `${props.opaEnv.prefix}-gitlab-hostname`,
      {
        allowedPattern: ".*",
    ...
    lbUrlParam = new ssm.StringParameter(
      this,
      `${props.opaEnv.prefix}-gitlab-url`,
      {
        allowedPattern: ".*",
