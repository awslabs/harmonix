...
export class NetworkConstruct extends Construct {
  ...
  constructor(parent: Construct, name: string, props: NetworkConstructProps) {
    ...
    const envIdentifier = `${props.opaEnv.prefix.toLowerCase()}-${
      props.opaEnv.envName
    }`;
    ...
    if (props.isIsolated) {
      ...
      // Create one eip less than the number of AZs'
      ...
      publicSubnetIds = JSON.stringify(
    ...
    const vpcPubSubnetParam = new ssm.StringListParameter(
      this,
      `${envIdentifier}-pub-subnet-param`,
      {
        allowedPattern: ".*",
    ...
    const vpcPriSubnetParam = new ssm.StringListParameter(
      this,
      `${envIdentifier}-priv-subnet-param`,
      {
        allowedPattern: ".*",
      ...
      for (const ip of props.allowedIPs) {
        if (ip.startsWith("pl-")) {
          ...
          allowedIpsSg.addIngressRule(
            ec2.Peer.ipv4(ip),
            ec2.Port.tcp(5432),
            "allow DB access from Allowed IP"
          );
          allowedIpsSg.addIngressRule(
            ec2.Peer.ipv4(ip),
            ec2.Port.tcp(80),
