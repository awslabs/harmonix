...
export class OPARootRoleConstruct extends Construct {
  ...
  constructor(
    parent: Construct,
    name: string,
    props: OPARootRoleConstructProps
  ) {
    super(parent, name);
    ...
    this.IAMRole = new iam.Role(this, name, {
      ...
      inlinePolicies: {
        [`${name}-role-assumption`]: this.createRoleAssumptionPolicyDocument(
          this.props.opaEnv.awsAccount
        ),
      },
    ...
    const roleParam = new ssm.StringParameter(
      this,
      `${this.props.opaEnv.prefix}-platform-role`,
      {
        allowedPattern: ".*",
  ...
  private createSecretsManagerStatement(): cdk.aws_iam.PolicyStatement {
    return new iam.PolicyStatement({
      ...
      resources: [
        `arn:aws:secretsmanager:*:${this.props.opaEnv.awsAccount}:secret:*`,
      ],
    });
  }
  ...
  private createResourceGroupsStatement(): cdk.aws_iam.PolicyStatement {
    return new iam.PolicyStatement({
      ...
      effect: iam.Effect.ALLOW,
      resources: [
        `arn:aws:resource-groups:*:${this.props.opaEnv.awsAccount}:group/*`,
      ],
    });
  }
  ...
  private createCloudformationStatement(): cdk.aws_iam.PolicyStatement {
    return new iam.PolicyStatement({
      ...
      effect: iam.Effect.ALLOW,
      resources: [
        `arn:aws:cloudformation:*:${this.props.opaEnv.awsAccount}:stack/${this.props.opaEnv.prefix}-api-*`,
      ],
    });
  }
  ...
  private createEc2Statement() {
    return new iam.PolicyStatement({
      ...
      effect: iam.Effect.ALLOW,
      resources: [
        `arn:aws:ec2:*:${this.props.opaEnv.awsAccount}:vpc/${this.props.network.vpc.vpcId}`,
      ],
    });
  }
  ...
  private createDyamodbStatement() {
    return new iam.PolicyStatement({
      ...
      effect: iam.Effect.ALLOW,
      resources: [
        `arn:aws:dynamodb:*:${this.props.opaEnv.awsAccount}:table/${this.props.securityTableName}`,
      ],
    });
  }

  private createRoleAssumptionPolicyDocument(
    account: string
    ...
    return new cdk.aws_iam.PolicyDocument({
      statements: [opaRoleAssumptionPolicy, operationsAssumptionPolicy],
    });
  }
}
